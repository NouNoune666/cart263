...
function createFlower() {
    ...
    // RETURN sends the flower object OUT of the function
    // It's like handing the finished product out of the workshop
    return flower;
}


// to render a flower (passed as an argument)
// PARAMETER: flower - this is the flower object passed IN when we call the function
function renderFlower(flower) {

    // Add CSS class "flower" to the stem div for styling
    flower.flowerStemDiv.classList.add("flower");

    // Set the stem's width using the stemThickness property
    // We join the number with "px" to create a valid CSS value
    // Example: if stemThickness is 10, this becomes "10px"
    flower.flowerStemDiv.style.width = flower.stemThickness + "px";
    flower.flowerStemDiv.style.height = flower.stemLength + "px";

    // Set the stem's background color using RGB values from the flower object
    ...
    // Position the stem horizontally (left edge of the stem)
    // flower.x is a random position we created in createFlower()
    flower.flowerStemDiv.style.left = flower.x + "px";

    // Position the stem vertically (top edge of the stem)
    flower.flowerStemDiv.style.top = flower.y - flower.stemLength + "px";

    // We append it to the grass div so flowers appear in the grass
